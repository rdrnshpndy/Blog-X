...
const CMSColleges = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim() || !location.trim() || !url.trim() || !type.trim()) {
    ...
    const collegeData = {
      ...
      courses: courses.trim().split(',').map(course => course.trim()), // Split comma-separated courses into an array
      photo: photo.trim(),
      url: url.trim(),
      nirfRank: parseInt(nirfRank.trim()),
      type: type.trim(),
    };
  ...
  return (
    <div className="bg-gray-300 min-h-screen w-full p-5">
      ...
      >
        {isCreateVisible ? 'Hide Section' : 'Create New Blog'}
      </button>
